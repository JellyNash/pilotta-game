...
const GameTable: React.FC = () => {
  ...
  const getPlayerByPosition = (position: string) => players.find(p => p.position === position);
  ...
  const handleCardPlay = async (card: CardType) => {
    if (isHumanTurn && phase === GamePhase.Playing && validMoves.some(c => c.id === card.id)) {
      ...
      announceToScreenReader(`You played ${card.rank} of ${card.suit}`);
    }
  };
  ...
  useEffect(() => {
    if (phase === GamePhase.Declaring && declarations.length > 0) {
      ...
      declarations.forEach(decl => {
        announceToScreenReader(`${decl.player.name} declared ${decl.type}`);
      });
      const timer = setTimeout(() => setShowDeclarations(false), 3000);
      return () => clearTimeout(timer);
    }
  }, [phase, declarations, announceToScreenReader]);
  ...
  useEffect(() => {
    if (isHumanTurn && phase === GamePhase.Playing) {
      announceToScreenReader(`Your turn. ${validMoves.length} playable cards.`);
    }
  }, [isHumanTurn, phase, validMoves.length, announceToScreenReader]);

  // Get table background based on settings
  const getTableBackground = () => {
    ...
    return feltColors[settings.comfort.tableFelt] || feltColors.green;
  };
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 bg-gradient-to-br ${getTableBackground()}`}>
        ...
        {settings.comfort.backgroundPattern !== 'none' && (
          <div 
            className="absolute inset-0 opacity-30"
            data-background-pattern={settings.comfort.backgroundPattern}
          />
        )}
      </div>

      {/* Table Border */}
      <div className="absolute inset-4 border-4 border-amber-900/50 rounded-3xl shadow-inner" />
      
      {/* Contract Indicator */}
      <ContractIndicator />

      {/* Game Content */}
      <div className="relative h-full flex flex-col" role="region" aria-label="Game area">
        {/* North Player (Partner) */}
        <div className="h-1/4 flex justify-center items-start pt-4" role="region" aria-label="North player area">
          {northPlayer && (
            <div 
            ...
            >
              ...
              />
              {/* Trick pile for teams */}
              {teams.A.trickPilePosition === 'north' && teams.A.wonTricks && teams.A.wonTricks.length > 0 && (
                <TrickPile tricks={teams.A.wonTricks} teamId="A" position="north" currentTrickNumber={trickNumber} />
              )}
              {teams.B.trickPilePosition === 'north' && teams.B.wonTricks && teams.B.wonTricks.length > 0 && (
                <TrickPile tricks={teams.B.wonTricks} teamId="B" position="north" currentTrickNumber={trickNumber} />
              )}
            </div>
          )}
        </div>
        ...
        {/* Middle Section */}
        <div className="flex-1 flex" role="region" aria-label="Table center">
          {/* West Player */}
          <div className="w-1/4 flex items-center justify-start pl-4" role="region" aria-label="West player area">
            {westPlayer && (
              <div 
                className="relative cursor-pointer"
              ...
              >
                ...
                />
                {/* Trick pile for teams */}
                {teams.A.trickPilePosition === 'west' && teams.A.wonTricks && teams.A.wonTricks.length > 0 && (
                  <TrickPile tricks={teams.A.wonTricks} teamId="A" position="west" currentTrickNumber={trickNumber} />
                )}
                {teams.B.trickPilePosition === 'west' && teams.B.wonTricks && teams.B.wonTricks.length > 0 && (
                  <TrickPile tricks={teams.B.wonTricks} teamId="B" position="west" currentTrickNumber={trickNumber} />
                )}
              </div>
            )}
          </div>
          ...
          {/* Center Play Area */}
          <div className="flex-1 flex items-center justify-center" role="region" aria-label="Play area">
          ...
          {/* East Player */}
          <div className="w-1/4 flex items-center justify-end pr-4" role="region" aria-label="East player area">
            {eastPlayer && (
              <div 
                className="relative cursor-pointer"
              ...
              >
                ...
                />
                {/* Trick pile for teams */}
                {teams.A.trickPilePosition === 'east' && teams.A.wonTricks && teams.A.wonTricks.length > 0 && (
                  <TrickPile tricks={teams.A.wonTricks} teamId="A" position="east" currentTrickNumber={trickNumber} />
                )}
                {teams.B.trickPilePosition === 'east' && teams.B.wonTricks && teams.B.wonTricks.length > 0 && (
                  <TrickPile tricks={teams.B.wonTricks} teamId="B" position="east" currentTrickNumber={trickNumber} />
                )}
              </div>
            )}
          </div>
        ...
        {/* South Player (Human) */}
        <div className="h-1/4 flex justify-center items-end pb-4" role="region" aria-label="Your hand">
          {southPlayer && (
            <div 
              className="relative"
              onClick={(e) => {
            ...
            >
              ...
              />
              {/* Trick pile for teams */}
              {teams.A.trickPilePosition === 'south' && teams.A.wonTricks && teams.A.wonTricks.length > 0 && (
                <TrickPile tricks={teams.A.wonTricks} teamId="A" position="south" currentTrickNumber={trickNumber} />
              )}
              {teams.B.trickPilePosition === 'south' && teams.B.wonTricks && teams.B.wonTricks.length > 0 && (
                <TrickPile tricks={teams.B.wonTricks} teamId="B" position="south" currentTrickNumber={trickNumber} />
              )}
            </div>
          )}
        </div>
        ...
        {earlyTermination && settings?.animations?.cardAnimations && (
          ...
          >
            ...
            <motion.div className="relative z-10">
              ...
              >
                <div className="bg-red-600/90 text-white px-8 py-4 rounded-xl shadow-2xl" role="alert">
